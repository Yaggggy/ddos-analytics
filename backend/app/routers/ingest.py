...
#         float(abuse.get("abuseConfidenceScore", 0)),
#         float(item.requests or 0),
#         float(item.bytes or 0)/1_000_000
#     )
#     geo = lookup(item.src_ip) or {}
#     inc = Incident(
#         src_ip=item.src_ip, dst=item.dst, vector=item.vector, bytes=item.bytes, requests=item.requests,
#         score=score, level=level, country=geo.get("country"), lat=geo.get("lat"), lon=geo.get("lon"),
#         details=f"abuse={abuse.get('abuseConfidenceScore', 0)}"
#     )
#     db.add(inc)
#     db.commit()
#     db.refresh(inc)
#     await manager.broadcast(inc.__dict__)
#     return inc
